...
ARGS['-m']           = ( ['--max-lines'],
                           {'default'   :None,
                            'type'      :int,
                            'help'      :"""(optional) The number of license file lines to prepend in the module doc strings"""})
...
def add_license(header,lic,sep=os.linesep,max_lines=None):
    if has_license(header):
...
if __name__ == '__main__':
    ...
    for current_file in python_files:
        ...
        if not args.remove:
            header = add_license(header,current_license,sep=sep,max_lines=args.max_lines)
